...
int change(int root,int a,int b)
{
    ...
    int r=node[root].r;
    int mid=(l+r)>>1;
    if(l==r) {node[root].sum=b;return b;}
    if(mid>=a) return node[root].sum=min(answer(root*2+1,mid+1,r),change(root*2,a,b));
    else return node[root].sum=min(answer(root*2,l,mid),change(root*2+1,a,b));
}
int answer(int root,int a,int b)
{
    int x=999999999;
